...
int sm3_test(void){
	unsigned char sm3in[15] = "flag{ling_tql}";
	unsigned char sm3out[SM3_HASH_SIZE];
	printf("\n``````````````````my sm3 test```````````````````\n");
	SM3Calc(sm3in,15,sm3out);
	printf("sm3: ");
	PrintBuf(sm3out,SM3_HASH_SIZE);	
    return 0;
}

void my_r(unsigned char *msg,unsigned char *out,int l){
    int i;
    for(i=0;i<l;i++){
        out[i] = msg[l-i-1];
    }
...
{	
	...
	unsigned char tx[15] = "flag{ling_tql}";
    ...
    //    unsigned char _std_priKey[32];
    //    unsigned char _std_Cipher[113];
    //    my_r(std_priKey,_std_priKey,32);
    //    my_r(std_Cipher,_std_Cipher,113);
...
int my_sm2_decrypt(void){
	unsigned char std_priKey[32]={	
		...
		0x9D,0x64,0x1B,0xDB,0xE4,0x7F,0x62,0xCC,0x57,0x4C,0xA3,0x49,0x4D,0x59,0xF0,0x75};
	unsigned char mtx[256] = {0};
	unsigned char std_Cipher[256];
	int len;

	HAL_UART_Receive(&huart1, std_Cipher, 1, 0xffff);

	if(len =  sm2_decrypt(std_Cipher,113,std_priKey,32,mtx) < 0){
		printf("sm2_decrypt error!\n");
		return 0;
	}else {
		// PrintBuf(mtx, len);
		HAL_UART_Transmit(&huart1, (uint8_t *)mtx, len, 0xffff);
	}	
	return 1;
}
